...
public:
	...
	const month_t FLAG_MONTH_IS_LEAP = 0x80;
...
// 	static WCHAR GetLunarYearEraZhi(WORD iLunarYear);
	...
	// 获取农历年的干支纪年
	//static void  GetLunarYearChineseEra(WORD iLunarYear, WCHAR& gan, WCHAR& zhi);
	static DWORD GetLunarYearChineseEra(WORD iLunarYear);

	//把iMonth格式化成中文字符串
	//static void GetMonthNameZh(WORD iMonth, WCHAR *pBuffer, BOOL bLunar);
	static DWORD GetMonthNameZh(WORD iMonth, BOOL bLunar);

	//把iDay格式化成中文字符串
	//static void GetDayNameZh(WORD iDay, WCHAR *pBuffer);
