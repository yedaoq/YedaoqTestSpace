...
int saveIcon(TCHAR* filename, TCHAR* iconFile) {
	...
	ATL::CComPtr<IShellFolder> shell_folder;
	SHGetDesktopFolder(&shell_folder);
	
	PIDLIST_ABSOLUTE pidl;
	HRESULT hr = shell_folder->ParseDisplayName(NULL, NULL, filename, NULL, &pidl, NULL);
	if(FAILED(hr))
	{
		printf("ParseDisplayName fail\n");
		return 0;
	}

	SHFILEINFO info;
	hr = SHGetFileInfo((LPCTSTR)pidl, NULL, &info, sizeof(info), SHGFI_PIDL | SHGFI_ICON | SHGFI_LARGEICON);
	if(FAILED(hr))
	{
		printf("SHGetFileInfo fail\n");
		return 0;
	}

	hIconLarge = info.hIcon;

	hr = SHGetFileInfo((LPCTSTR)pidl, NULL, &info, sizeof(info), SHGFI_PIDL | SHGFI_ICON | SHGFI_SMALLICON);
	if(FAILED(hr))
	{
		printf("SHGetFileInfo fail\n");
		return 0;
	...
	hIconSmall = info.hIcon;
	
	ret = SaveIcon3(iconFile, &hIconSmall, 2);
	if ( ret ) {
		return 0;
	}
	return -1;
